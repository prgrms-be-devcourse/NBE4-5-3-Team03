...
export default function ClientPage({
  ...
  data: components["schemas"]["PageDtoActorDto"];
  keyword: string;
  page: number;
  pageSize: number;
}) {
  ...
  // 검색 실행 함수
  const handleSearch = () => {
    router.push(`/actors?keyword=${searchKeyword}&page=1&pageSize=${pageSize}`);
  };
  ...
  const handlePageSizeChange = (value: string) => {
    router.push(`/actors?keyword=${keyword}&page=1&pageSize=${value}`);
  };

  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold mb-6 text-center">🎭 배우 목록</h1>
      {/* 검색 바 + 페이지 크기 선택 */}
      ...
      <div className="flex justify-center mt-8 space-x-2">
        {!isFirstGroup && (
          <Link
            href={`/actors?keyword=${keyword}&page=${prevPage}&pageSize=${pageSize}`}
          >
        ...
        {Array.from(
          ...
          <Link
            key={pageNo}
            href={`/actors?keyword=${keyword}&page=${pageNo}&pageSize=${pageSize}`}
          >
            <Button
        ...
        {!isLastGroup && (
          <Link
            href={`actors?keyword=${keyword}&page=${nextPage}&pageSize=${pageSize}`}
          >
            <Button variant="outline" className="cursor-pointer">
              다음
            </Button>
